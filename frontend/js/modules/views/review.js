...
export function createReviewView({ api, apiContract, generateReport, onAskCoach, showView }) {
  ...
  function renderReviewWorkspace() {
    ...
    <span class="review-pill inline-flex items-center rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-2 py-[3px] text-[11px] text-[var(--muted)]">${move.phase || 'phase unknown'}</span>
    <span class="review-pill inline-flex items-center rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-2 py-[3px] text-[11px] text-[var(--muted)]">${move.classification || 'unclassified'}</span>
    <span class="review-pill inline-flex items-center rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-2 py-[3px] text-[11px] text-[var(--muted)]">${move.is_hanging_piece ? 'hanging piece' : 'piece safe'}</span>
  `;
  ...
  function renderJournalMarkup(journal) {
    if (journal) {
      return `
  <div class="card mt-2 rounded-cc border border-[var(--border)] bg-[var(--surface)]">
...
`;
    ...
    return `
  <div class="card mt-2 rounded-cc border border-[var(--border)] bg-[var(--surface)] p-4">
...
`;
      ...
      <section class="review-stage rounded-cc border border-[var(--border)] bg-[var(--surface)] p-5">
        <div class="review-header flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <div class="review-title mt-1 text-[28px] leading-none tracking-[-0.03em]" id="review-title">Move</div>
        ...
        <div class="review-board-shell mt-2">
  ...
  reviewState.playerMoves.length
      ...
      <div class="mistake-panel flex flex-col gap-1">
        <div class="mistake-panel-title">
